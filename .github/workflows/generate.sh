...
export BASHBREW_NAMESPACE='khulnasoft'
			...
			newStrategy="$(jq -c '
				.matrix.include[].runs.build |= (
					...
					| [
						"[ -s true/\($binary) ]",
						"rm -v true/\($binary)",
						"docker build --pull --tag khulnasoft/true:builder --target asm --file true/Dockerfile.all true",
						"docker run --rm khulnasoft/true:builder tar -cC /true \($binary) | tar -xvC true",
